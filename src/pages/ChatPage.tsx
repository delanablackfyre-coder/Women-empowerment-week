...
import { Send, Paperclip, ShieldCheck, Gavel, Heart, Info } from 'lucide-react';
...
export default function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "Assalomu alaykum. Men Najot AI yordamchisiman. Sizga qanday yordam bera olaman? Sizning xavfsizligingiz va anonimligingiz biz uchun birinchi o'rinda." }
  ]);
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] md:h-screen max-w-4xl mx-auto">
      <header className="p-4 border-b border-outline-variant/20 flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-on-surface">Najot AI</h1>
        ...
        {messages.map((msg, i) => (
          ...
          >
            <div className={cn(
              ...
              {msg.role === 'model' ? <ShieldCheck size={16} /> : <Heart size={16} />}
            </div>
            <div className={cn(
      ...
      <div className="p-4 bg-white/50 backdrop-blur-md border-t border-outline-variant/20">
        ...
        <p className="text-[10px] text-center text-outline-variant mt-3 font-medium">
          Najot AI xatoliklar qilishi mumkin. Muhim qarorlar uchun mutaxassis bilan maslahatlashing.
        </p>
      </div>
    </div>
  );
